...
`ls {product_specs_dir}/*<feature>*`. Spec exists -> advance. Spec missing
...
**On disk:** `{product_specs_dir}` (Robert's specs, living), `{ux_docs_dir}` (Sable's UX docs, living), `{architecture_dir}` (Cal's ADRs, immutable), `{conventions_file}`, `{pipeline_state_dir}`, `{architecture_dir}/README.md` (ADR index), `{changelog_file}`, code, tests, and Agatha's docs.
