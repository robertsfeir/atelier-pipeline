...
ALWAYS sections live in this file: Brain Access, Mandatory Gates, Terminal
Transition, Agent Output Masking, Subagent Invocation & DoR/DoD, Pipeline
Flow, Agent Standards, CI Watch. JIT sections live in sibling references;
Eva reads only the referenced file on the listed trigger.

| JIT Section | Trigger | Location |
|-------------|---------|----------|
| Telemetry Capture (+ Darwin Auto-Trigger, Pattern Staleness, Dashboard Bridge) | After first agent returns / pipeline end | `telemetry-metrics.md` (`<protocol id="telemetry-capture">`) |
| Investigation Discipline | Debug flow entered | `pipeline-phases.md` (`<protocol id="investigation">`) |
| Concurrent Session Detection | Session boot with stale active state | `pipeline-phases.md` (`<protocol id="concurrent-session-hard-pause">`) |
| State File Descriptions | First state write | `pipeline-phases.md` (`<section id="state-files">`) |
| Phase Sizing Rules | Pipeline sizing decision | `pipeline-phases.md` (`<section id="phase-sizing">`) |
| Budget Estimate Gate | Pipeline sizing decision | `pipeline-phases.md` (`<gate id="budget-estimate">`) |
| Worktree-Per-Session Isolation | Pipeline start (before Colby) / Ellis cleanup | `worktree-isolation.md` (`<protocol id="worktree-per-session">`) |
...
**Agent Teams interactions (gates 2, 3, 5):** When Agent Teams is active,
Teammates execute builds and mark tasks complete but do NOT commit. Eva
merges each Teammate's worktree into the working branch sequentially, then
routes Ellis for per-unit commits on the integrated result. Teammates run
lint but not the full suite -- Eva runs the full test suite (gate 3) on the
integrated codebase after each wave's merge. Poirot (gate 5) blind-reviews
the cumulative wave diff after all Teammate worktrees are merged.
...
**Extension rule:** New stop reasons are added by a new ADR that supersedes
the enum table above. Eva never adds values at runtime. `legacy_unknown` is
read-only -- a read-time inference for pre-ADR-0028 files that lack the
field entirely; Eva writes only the nine active values.
