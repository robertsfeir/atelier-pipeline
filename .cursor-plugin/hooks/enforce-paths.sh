...
case "$AGENT_TYPE" in
  sarah)
    path_matches "$FILE_PATH" "$ARCH_DIR" || {
      echo "BLOCKED: Sarah can only write to $ARCH_DIR/. Route documentation to Agatha, source code to Colby. Attempted: $FILE_PATH" >&2
      exit 2
    }
    ;;
  ...
  "")
    ...
    path_matches "$FILE_PATH" "$PIPELINE_DIR" "$PRODUCT_DIR" "$UX_DIR" || {
      echo "BLOCKED: Main thread (Eva/Robert/Sable) can only write to $PIPELINE_DIR/, $PRODUCT_DIR/, or $UX_DIR/. Route source code changes to Colby, architecture to Sarah, documentation to Agatha. Attempted: $FILE_PATH" >&2
      exit 2
    }
    ;;
