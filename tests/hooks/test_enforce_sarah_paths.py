"""Tests for ADR-0033 Step 5 (m2): enforce-sarah-paths.sh dead MultiEdit branch removal.
...
HOOK = PROJECT_ROOT / "source" / "claude" / "hooks" / "enforce-sarah-paths.sh"
...
def test_T_0033_025_sarah_case_statement_no_multiedit():
    """enforce-sarah-paths.sh TOOL_NAME case statement must match Write|Edit only.
    ...
    """
    assert HOOK.exists(), f"enforce-sarah-paths.sh missing at {HOOK}"
    text = HOOK.read_text()
    ...
    assert case_lines, (
        "Could not find a `case \"$TOOL_NAME\" in ...` line in enforce-sarah-paths.sh"
    )

    # Assert MultiEdit does not appear on the case line.
    for line in case_lines:
        assert "MultiEdit" not in line, (
            f"enforce-sarah-paths.sh still has dead MultiEdit branch in case statement:\n"
            f"  {line}\n"
