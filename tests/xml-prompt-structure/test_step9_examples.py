...
def test_T_0005_131_examples_between_workflow_and_tools():
    ...
    for f in AGENT_FILES:
        ...
        if not all([workflow_end, examples_start, examples_end, tools_start]):
            errors.append(f"Missing tags in {f}")
            continue
        if workflow_end >= examples_start:
            errors.append(f"In {f}: </workflow> should come before <examples>")
        if examples_end >= tools_start:
            errors.append(f"In {f}: </examples> should come before <tools>")
    assert not errors, "\n".join(errors)
...
def test_T_0005_132_at_least_2_examples():
    ...
    for f in AGENT_FILES:
        ...
        if count < 2:
            errors.append(f"Only {count} examples in {f} (need at least 2)")
    assert not errors, "\n".join(errors)
...
def test_T_0005_134_brain_agents_reference_brain_in_examples():
    ...
    for agent in BRAIN_AGENTS:
        ...
        if not file.is_file():
            continue
        examples = extract_tag_content(file, "examples")
        if not re.search(r"brain.context|brain context|prior.*decision|injected.*thought|prior.*pattern|brain-context", examples, re.IGNORECASE):
            errors.append(f"Missing brain context reference in examples of {f}")
    assert not errors, "\n".join(errors)
    ...
    for f in AGENT_FILES:
        ...
        for line in examples.splitlines():
            if re.match(r"^\s*\*\*", line):
                if in_scenario and current_lines > 5:
                    errors.append(f"Example '{scenario_header.strip()}' in {f} has {current_lines} lines (max 5)")
...
def test_T_0005_141_examples_demonstrate_tool_usage():
    ...
    for f in AGENT_FILES:
        ...
        if not file.is_file():
            continue
        examples = extract_tag_content(file, "examples")
        if not re.search(r"Read|Grep|Glob|Bash|Write|Edit|grep|read|check|verify", examples, re.IGNORECASE):
    ...
    for f in AGENT_FILES:
        ...
        for line in examples.splitlines():
            if re.match(r"^\s*\*\*", line):
                ...
                if re.search(r"\bRead\b|\bGrep\b|\bGlob\b|\bBash\b|\bWrite\b|\bEdit\b|git diff|git log", line, re.IGNORECASE):
