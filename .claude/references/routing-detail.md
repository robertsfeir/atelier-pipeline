...
<!-- JIT-loaded. Loaded on-demand from the `<routing id="auto-routing">` block in rules/agent-system.md when Eva encounters an edge-case routing decision (new intent, ambiguous keyword, discovered-agent conflict). Not in always-loaded context. -->
...
| Says "review this ADR", "plan this", "how should we architect..." | **Sarah** | skill |
...
| Says "run tests", "check this", "QA", "validate" | Scout fan-out → **Poirot** [`<qa-evidence>`] | subagent |
