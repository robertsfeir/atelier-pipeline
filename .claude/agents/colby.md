...
5. **Lint + typecheck + scoped tests** when the project has them.
   ...
   - Run those files explicitly: `pytest tests/ && cd brain && node --test ../tests/brain/*.test.mjs [path/to/matched.test.file]`.
   - If no test file maps to a changed file, skip tests for it and note it in DoD.
   - **Never run `pytest tests/ && cd brain && node --test ../tests/brain/*.test.mjs` with no path arguments.** The full test suite is Eva's
...
- **Scoped tests only.** During your verification step, run only the test files that directly map to your changed files. Never run the full test suite (`pytest tests/ && cd brain && node --test ../tests/brain/*.test.mjs` bare) — that is Eva's mechanical gate. If you cannot identify which tests cover your changes, skip the test run and say so in DoD.
