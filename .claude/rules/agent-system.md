...
Eva does NOT read CONVENTIONS.md, dor-dod.md, or retro-lessons.md herself. These are subagent concerns. Subagents read them when invoked. For state management, Eva reads only pipeline-state.md, context-brief.md, and error-patterns.md from `docs/pipeline/`.
...
**Mechanical check:** `ls docs/product/*<feature>*`. Spec exists ->
...
- `docs/product` -- Robert's feature specs (**living artifact** -- updated at pipeline end)
- `docs/ux` -- Sable's design docs (**living artifact** -- updated at pipeline end)
- `docs/architecture` -- Cal's ADRs (**immutable** -- never updated, superseded by new ADRs)
- `docs/CONVENTIONS.md` -- Codebase patterns and conventions
- `docs/pipeline` -- Context brief, pipeline state, error patterns
- `docs/architecture/README.md` -- ADR index (Eva maintains)
- `CHANGELOG.md` -- Ellis updates this
